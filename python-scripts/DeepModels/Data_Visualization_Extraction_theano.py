       ...
       if NN not in models:
          ...
          for i in target_layer:
                ...
                if Visualization[0]=="all":
                      ...
                      ## kernel weights
                      util.iftFilterToDataset(intermediate_output, Y_t, Y_p, setup['num_classes'], NN+"_iftFilterProjection.zip", False, backend="theano", filter_in=layer_weights, bias = bias_weights)  
                      ## histogram of activations
                      util.iftFilterToDataset(intermediate_output, Y_t, Y_p, setup['num_classes'], NN+"_iftHistogramProjection.zip", False, backend="theano", filter_in=None, bias = None) 

                      num_samples = 8
                      ## activations per image/per kernel 
                      util.iftActivationsPerFilter(intermediate_output, Y_t, Y_p, setup['num_classes'], NN+"_iftActivationsPerFilter.zip", num_samples, False, backend="theano")  

                      ## activations per kernel
                      util.iftActivationsPerFilter_simplified(intermediate_output, Y_t, Y_p, setup['num_classes'], NN+"_iftSimplifiedActivationsPerKernel.zip", num_samples, False, backend="theano")  

                       ## Selectivity                  
                      util.iftSelectivity(intermediate_output, Y_t, setup['num_classes'], NN+"_iftSelectivity.zip", backend = "theano")
                else:
